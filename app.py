...
def recipes():
    ...
    # Get filter parameters from query string (?search=...&tag=...)
    search = request.args.get('search', '').strip()
    tag_filter = request.args.get('tag', '').strip()

    conn = get_db_connection()

    # Base query with joins so we can filter by tag
    query = '''
        SELECT DISTINCT r.recipe_id, r.name, r.instructions, r.prep_time_minutes, 
               r.cost_estimate, r.created_at,
               i.file_path AS image_url, i.alt_text AS image_alt,
               4.5 AS avg_rating,
               0 AS review_count,
    ...
    return render_template(
        ...
        current_ingredient=search,  # if your template still uses this
        all_tags=all_tags
    )
